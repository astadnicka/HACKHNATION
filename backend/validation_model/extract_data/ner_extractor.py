...
class NERExtractor:
    ...
    def identify_document_type(self, text):
        ...
        if "opinia" in lower_text and "kwalifikacji" in lower_text: return "opinia"
        if "zapis wyjaśnień" in lower_text or "wyjaśnień poszkodowanego" in lower_text: return "wyjasnienie"
        if "karta wypadku" in lower_text: return "karta_wypadku"
        if "zawiadomienie o wypadku" in lower_text or "ewyp" in lower_text: return "zawiadomienie"
        return "unknown"
    ...
    def _extract_date(self, text):
        ...
        if context_match:
             ...
             if not any(ign in value for ign in self.ignored_dates): return {"value": value}
        ...
        valid_matches = [m.replace(" ", "").replace("r.", "").strip() for m in matches if not any(ign in m for ign in self.ignored_dates)]
        return {"value": valid_matches[0]} if valid_matches else {"value": None}
    ...
    def _extract_person(self, text):
        ...
        if pesel_match: person_data["pesel"] = pesel_match.group(0)
    ...
    def _extract_description(self, text, doc_type):
        ...
        if doc_type == "zawiadomienie":
            start_keywords = ["5. szczegółowy opis", "szczegółowy opis", "okoliczności, miejsca i przyczyn"]
        elif doc_type == "wyjasnienie":
            start_keywords = ["oświadczam, co następuje", "okoliczności i przyczyn wypadku"]
        elif doc_type == "opinia":
            start_keywords = ["uzasadnienie", "stan faktyczny", "opis zdarzenia"]
        else:
            start_keywords = ["okoliczności wypadku", "opis zdarzenia"]
        ...
        if start_idx != -1:
            end_idx = -1
            possible_ends = ["\n6.", "\nCzy", "6. Czy", "\nŚwiadkiem", "--- Page", "wniosek", "podpis", "data"]
            min_dist = 5000
            for end_marker in possible_ends:
                found_end = lower_text.find(end_marker.lower(), start_idx + 20)
            ...
            clean_patterns = [
                r"5\.\s*Szczegółowy opis.*?\n",
                r"Podaj szczegółowy opis.*?\n",
                r"okoliczności,?\s*miejsca\s*[i|l1]\s*przyczyn.*?\n", 
                r"miejsca\s*[i|l1]\s*przyczyn\s*wypadku",
                r"\(opis, przyczyny techniczne.*?\)",
                r"- należy uzupełnić.*?\n",
                r"Sklad: Poligrafia.*",
                r"Strona \d z \d"
            ]
            ...
            for pat in clean_patterns:
                description = re.sub(pat, "", description, flags=re.IGNORECASE | re.DOTALL)
    ...
    def _extract_injury(self, text):
        ...
        for kw in keywords:
            idx = lower_text.find(kw)
            if idx != -1: start_idx = idx + len(kw); break
        if start_idx != -1:
            end_idx = -1
            ...
            for end_kw in possible_ends:
                ...
                if found != -1 and (found - start_idx) < min_dist: min_dist = found - start_idx; end_idx = found
            if end_idx != -1: injury = text[start_idx:end_idx].strip()
            else: injury = text[start_idx:start_idx+200].strip()
            injury = injury.strip(" :.-_\n")
        return {"value": injury}

    def _extract_conclusion(self, text, doc_type):
        if doc_type != "opinia": return {"value": None}
        lower_text = text.lower()
        start_keywords = ["wniosek:", "opinia:", "wnoszę o:", "proponuję:"]
        start = -1
        for kw in start_keywords:
            idx = lower_text.find(kw)
            if idx != -1: start = idx + len(kw); break
        if start != -1:
            end_keywords = ["uzasadnienie", "podpis", "data", "--- page"]
            end = -1; min_dist = 2000
            for kw in end_keywords:
                idx = lower_text.find(kw, start)
                if idx != -1 and (idx - start) < min_dist: min_dist = idx - start; end = idx
            if end != -1: return {"value": text[start:end].strip()}
            else: return {"value": text[start:start+500].strip()}
        return {"value": None}