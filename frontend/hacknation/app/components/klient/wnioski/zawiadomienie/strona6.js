...
export default function Strona6() {
  ...
  return (
    <div className="relative bg-gray-50/60 w-full max-w-2xl p-4 pb-6 rounded-xl flex flex-col space-y-4 mb-4">
      ...
      <div className="grid grid-cols-5 gap-2">
        <input className="h-4 w-4 rounded border-gray-300 text-cyan-500 focus:ring-cyan-500" type="checkbox" checked={odbiorPlacowka} onChange={(e) => setOdbiorPlacowka(e.target.checked)} />
        <label className="col-span-4">w placówce ZUS (osobiście lub przez osobę upoważnioną)</label>
      </div>
      <div className="grid grid-cols-5 gap-2">
        <input className="h-4 w-4 rounded border-gray-300 text-cyan-500 focus:ring-cyan-500" type="checkbox" checked={odbiorPoczta} onChange={(e) => setOdbiorPoczta(e.target.checked)} />
        <label className="col-span-4">pocztą na adres wskazany we wniosku</label>
      </div>
      <div className="grid grid-cols-5 gap-2">
        <input className="h-4 w-4 rounded border-gray-300 text-cyan-500 focus:ring-cyan-500" type="checkbox" checked={odbiorPUE} onChange={(e) => setOdbiorPUE(e.target.checked)} />
      ...
      <div className="grid grid-cols-2 gap-2">  
        ...
        <textarea value={podpis} onChange={(e) => setPodpis(e.target.value)} />
      </div>
